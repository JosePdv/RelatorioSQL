...
$sql = $conn->prepare("SELECT nome as nome, protocolo   as protocolo,

       SUM(ponc_graduacao + ponc_doutorado + ponc_mestrado 
       + ponc_posgraduacao1 + ponc_posgraduacao2 
       + ponc_qualific1 +
           ponc_qualific2 + ponc_qualific3 + 
           ponc_qualific4 + ponc_qualific5 + 
           ponc_espe1 + ponc_espe2 + 
           ponc_espe3 +ponc_espe4)  as nota, SUM(ponc_espe1 + ponc_espe2 + 
           ponc_espe3 + ponc_espe4)  as experiencia
       
FROM ponctuations LEFT JOIN formularios ON ponctuations.formulario_id = formularios.id
WHERE aprovado = 1 and cargo = 10
GROUP BY nome,  protocolo ORDER By nota desc ,experiencia desc, data_nascimento asc;");
...
para que produzam os devidos e legais efeitos, como segue:</p>
				...
				<h5 style="text-align: center;">CARGO CIRURGIÃO DENTISTA </h5>
